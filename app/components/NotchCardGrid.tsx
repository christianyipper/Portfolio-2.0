...
export interface NotchCardEntry {
    title: string;
    client?: string;
    logo?: string;
    logoAlt?: string;
...
function NotchCard({ entry, visible, clipId }: { entry: NotchCardEntry; visible: boolean; clipId: string }) {
...
export default function NotchCardGrid<T extends NotchCardEntry>({
    heading,
    entries,
    hrefFor,
    keyPrefix,
    ...
    heading: string;
    entries: T[];
    hrefFor: (entry: T, index: number) => string;
    keyPrefix: string;
}) {
    const { navigate } = useTransition();
    const cardRefs = useRef<(HTMLDivElement | null)[]>([]);
    const [visibleStates, setVisibleStates] = useState<boolean[]>(new Array(entries.length).fill(false));
                ...
                >
                    {entries.map((entry, i) => {
                        ...
                        return (
                            ...
                            >
                                <NotchCard entry={entry} visible={visibleStates[i]} clipId={`notch-card-clip-${keyPrefix}-${i}`} />
                            </Link>
                        );
                    })}
                </div>
